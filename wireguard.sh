...
Address = 192.168.3.1/24, fd10:db31:203:ab31::1/64 
...
AllowedIPs = 192.168.3.2/32, fd10:db31:203:ab31::2/64" > wg0.conf
...
# VPN内网IP范围
Address = 192.168.3.2/24,fd10:db31:203:ab31::2/64
# 解析域名用的DNS
DNS = 119.29.29.29, 2402:4e00::
# 保持默认
MTU = 1300
...
[Peer]
PublicKey = $(cat cpublickey)
AllowedIPs = 192.168.3.2/32, fd10:db31:203:ab31::2
EOF
...
# 手动指定每个用户的IP地址（包括IPv6）
user_ipv4=("192.168.3.2" "192.168.3.3" "192.168.3.4" "192.168.3.5" "192.168.3.6" "192.168.3.7" "192.168.3.8" "192.168.3.9")
user_ipv6=("fd10:db31:203:ab31::2" "fd10:db31:203:ab31::3" "fd10:db31:203:ab31::4" "fd10:db31:203:ab31::5" "fd10:db31:203:ab31::6" "fd10:db31:203:ab31::7" "fd10:db31:203:ab31::8" "fd10:db31:203:ab31::9")

# 添加多用户配置
for ((i=0; i<${#user_ipv4[@]}; i++))
do
    ipv4=${user_ipv4[$i]}
    ipv6=${user_ipv6[$i]}

    # 生成客户端私钥并添加对应的公钥到服务器配置文件
    wg genkey | tee cprivatekey | wg pubkey > cpublickey
